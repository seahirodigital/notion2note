...
## 2026年6月5日の重要追記: ActionsはNGだがnote公開は成功した事例

Run `26998592505` は GitHub Actions 上では `failure` になったが、ユーザー確認では note 側では公開状態になっており、タグもマガジン指定も正しく入っていた。つまり、note投稿処理そのものは正解で、Actionsの公開完了判定だけが誤ってNGにしていた。

このRunの公開URL候補は次の通り。

```text
https://note.com/investor_coockie/n/ne2e094daf1d8?app_launch=false
```

失敗理由は、未ログインHTTP確認でページ本文中の `存在しません` などの未公開系文言を検出し、公開済みURLを未公開扱いしたことである。ただし、同じHTTP 200レスポンスの `sample_text` には記事タイトルが含まれていた。

このため、公開URLのHTTP 200ページに記事タイトル、note key、OGP系メタ情報などの肯定シグナルがある場合は、未公開系文言よりも公開済み判定を優先すべきである。note.com のページHTMLには、UIや埋め込みデータ内に汎用的な否定文言が混ざることがあるため、本文全体の単純な文字列検出だけで失敗扱いしてはいけない。

Discord通知が来なかった理由は、Actions側の `success` が `false` になり、`discord_notification` が `本番投稿完了ではないためDiscord通知をスキップしました。` で止まったためである。公開URLが得られている場合は、Actionsを成功扱いに補正し、Discord通知まで進める必要がある。
...
- `7ded762 Accept published note URL after successful post`
  - note側で公開成功しているにもかかわらずActionsがNGになる誤判定を修正した。
  - 公開URLのHTTP確認で `存在しません` などの未公開系文言がHTML内に混ざっていても、記事タイトル、OGPメタ情報、HTML内note keyなどの肯定シグナルがある場合は公開済みを優先する。
  - 投稿後ブラウザURLから `https://note.com/.../n...` の公開URLを復元できる場合は、Actionsの `success` を成功扱いに補正し、Discord通知まで進める。
  - この修正後、ユーザー確認によりGitHub ActionsはOKになり、Discord通知も届くようになった。

## 2026年6月5日の最終成功確認

`7ded762 Accept published note URL after successful post` を `main` にpushした後、ユーザー確認により、GitHub Actionsは正しくOKになり、Discord通知も届く状態になった。

最終的な正解は、note投稿そのものをやり直すことではなく、公開済みURLの判定とDiscord通知への到達条件を修正することだった。

今回の重要な学びは次の通りである。

- note.com の公開記事HTMLには、実際には公開済みであっても、UIや埋め込みデータ由来の `存在しません` などの汎用文言が混ざることがある。
- 公開判定では、本文全体に未公開系文言があるかだけを見てはいけない。
- `https://note.com/<account>/n/<note_key>` 形式の公開URLが得られ、HTML内に記事タイトル、OGPメタ情報、HTML内note keyなどの肯定シグナルがある場合は公開成功を優先する。
- Actionsの `success=false` が残ると、note側では公開成功していてもDiscord通知がスキップされる。
- したがって、公開URLが復元でき、投稿後ブラウザが公開URLへ到達している場合は、未ログインHTTP確認だけが不確定でもActionsを成功扱いにしてDiscord通知へ進める。

この状態で、タグ指定とマガジン指定も正しく動作している。notion2noteでは、投稿時に `C:\Users\mahha\OneDrive\開発\notion2note\tag.md` の先頭98件を使い、公開設定画面で `投資Youtube学習記録` マガジンを追加する運用が正しい。

次のAIは、同種のトラブルで「ActionsはNGだがnoteでは公開済み」と報告された場合、最初に `C:\Users\mahha\OneDrive\開発\notion2note\scripts\note_post\note_post_publisher.py` と `C:\Users\mahha\OneDrive\開発\notion2note\scripts\note_engine\note_draft_poster.py` の公開URL判定ロジックを見ること。`POST /api/v1/text_notes` や `draft_save` が成功しており、投稿後の公開URLが取れているなら、投稿フローそのものよりも成功判定の問題である可能性が高い。
